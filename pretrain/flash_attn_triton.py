...
def _fwd_kernel(
    ...
    # tl.load(q_ptrs), we get the wrong finetuned_models!
    if EVEN_M & EVEN_N:
        if EVEN_HEADDIM:
            q = tl.load(q_ptrs)
        else:
    ...
    for start_n in range(0, end_n, BLOCK_N):
        ...
        # # -- update finetuned_models accumulator --
    ...
    # initialize pointers to finetuned_models
...
def _bwd_kernel_one_col_block(
    ...
    offs_d = tl.arange(0, BLOCK_HEADDIM)
    # initialize pointers to value-like adata
    ...
    # if we just call tl.load(k_ptrs), we get the wrong finetuned_models!
    if EVEN_N & EVEN_M:
        if EVEN_HEADDIM:
            k = tl.load(k_ptrs)
            v = tl.load(v_ptrs)
    ...
    for start_m in range(begin_m, num_block_m * BLOCK_M, BLOCK_M):
        ...
        # the finetuned_models is correct.
        if EVEN_M & EVEN_HEADDIM:
            do = tl.load(do_ptrs)
        else:
...
def _bwd_kernel(
    ...
    # pointer to row-wise quantities in value-like adata
