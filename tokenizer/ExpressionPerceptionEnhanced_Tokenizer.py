...
ENSEMBL_MAPPING_FILE = BASE_DIR / "./token_files/gene_to_ensembl_mapping_95m.json"
    ...
    def create_dataset_with_expression(
        ...
        try:
            if use_generator:
                ...
                dataset_dict = {"input_ids": [], "expression_values": []}
                for example in generator_func():
                    dataset_dict["input_ids"].append(example["input_ids"])
                    dataset_dict["expression_values"].append(example["expression_values"])
                output_dataset = Dataset.from_dict(dataset_dict)
        except Exception as e:
