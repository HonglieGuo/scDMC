...
| Single-cell transcriptomes as Geneformer rank value encodings with cell state labels in Geneformer .dataset format (generated from single-cell RNAseq adata by tokenizer.py)

| Gene classifier:
| Dictionary in format {Gene_label: list(genes)} for gene labels and single-cell transcriptomes as Geneformer rank value encodings in Geneformer .dataset format (generated from single-cell RNAseq adata by tokenizer.py)
...
class Classifier:
    ...
    def __init__(
        ...
        **Parameters:**
        ...
        cell_state_dict : None, dict
            ...
            | Alternatively, instead of a list of states, can specify "all" to use all states in that state key from input adata.
            | Of note, if using "all", states will be defined after adata is filtered.
        ...
        filter_data : None, dict
            | Default is to fine-tune with all input adata.
        ...
        num_crossval_splits : {0, 1, 5}
            | 0: train on all adata without splitting
            | 1: split adata into train and eval sets by designated split_sizes["valid"]
            | 5: split adata into 5 folds of train and eval sets by designated split_sizes["valid"]
        split_sizes : None, dict
            | Dictionary of proportion of adata to hold out for train, validation, and test sets
        ...
        no_eval : bool
            | If True, will skip eval step and use all adata for training.
    ...
    def prepare_data(
        ...
        """
        Prepare adata for cell state or gene classification.
        ...
        **Parameters**
        ...
        output_directory : Path
            | Path to directory where prepared adata will be saved
        output_prefix : str
            | Prefix for finetuned_models file
        split_id_dict : None, dict
            | Dictionary of IDs for train and test splits
            | Three-item dictionary with keys: attr_key, train, test
            | attr_key: key specifying name of column in .dataset that contains the IDs for the adata splits
        ...
        test_size : None, float
            | Proportion of adata to be saved separately and held out for test set
        ...
        attr_to_split : None, str
            | Key for attribute on which to split adata while balancing potential confounders
            | e.g. "patient_id" for splitting by patient while balancing other characteristics
            | Note: only available for CellClassifiers
        attr_to_balance : None, list
            | List of attribute keys on which to balance adata while splitting on attr_to_split
        ...
        # prepare adata and labels for classification
    ...
    def train_all_data(
        ...
        """
        Train cell state or gene classifier using all adata.
        ...
        **Parameters**
        ...
        output_directory : Path
            | Path to directory where model and eval adata will be saved
        output_prefix : str
            | Prefix for finetuned_models files
        save_eval_output : bool
            | Whether to save cross-fold eval finetuned_models
            | Saves as pickle file of dictionary of eval metrics

        **Output**

        Returns trainer after fine-tuning with all adata.

        """

        ##### Load adata and prepare finetuned_models directory #####
        # load numerical id to class dictionary (id:class)
        with open(id_class_dict_file, "rb") as f:
            id_class_dict = pickle.load(f)
        class_id_dict = {v: k for k, v in id_class_dict.items()}

        # load previously filtered and prepared adata
        data = pu.load_and_filter(None, self.nproc, prepared_input_data_file)
        data = data.shuffle(seed=42)  # reshuffle in case users provide unshuffled adata

        # define finetuned_models directory path
        ...
        **Parameters**
        ...
        if self.classifier == "cell":
            ...
            for train_index, eval_index, test_index in tqdm(
                ...
                ksplit_output_dir = os.path.join(output_dir, f"ksplit{iteration_num}")
                # filter adata for examples containing classes for this split
                # subsample to max_ncells and relabel adata in column "labels"
    ...
    def hyperopt_classifier(
        ...
        **Parameters**
        ...
        if os.path.isfile(saved_model_test) is True:
            logger.error("Model already saved to this designated finetuned_models directory.")
            raise
        # make finetuned_models directory
    ...
    def evaluate_model(
        ...
        **Parameters**
        ...
        predict : bool
            | Whether or not to save eval predictions
        output_directory : Path
            | Path to directory where eval adata will be saved
        output_prefix : str
            | Prefix for finetuned_models files
        """
    ...
    def plot_conf_mat(
        ...
        **Parameters**
        ...
        custom_class_order : None, list
            ...
            | Same order will be used for all pretrained_models.
        """
    ...
    def plot_predictions(
        ...
        **Parameters**
        ...
        predictions_file : path
            | Path of model predictions finetuned_models to plot
            | (saved finetuned_models from self.validate if predict_eval=True)
            | (or saved finetuned_models from self.evaluate_saved_model)
        id_class_dict_file : Path
            | Path to _id_class_dict.pkl previously prepared by Classifier.prepare_data
            | (dictionary of format: numerical IDs: class_labels)
        title : str
        ...
        if isinstance(predictions, dict):
            if all(
                ...
                # format is finetuned_models from self.evaluate_saved_model
                predictions_logits = np.array(predictions["predictions"])
                true_ids = predictions["label_ids"]
        else:
            # format is finetuned_models from self.validate if predict_eval=True
