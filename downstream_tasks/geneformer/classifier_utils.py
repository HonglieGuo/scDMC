...
def validate_and_clean_cols(train_data, eval_data, classifier):
    # validate that adata has expected label column and remove others
    if classifier == "cell":
        label_col = "label"
...
def prep_gene_classifier_split(
    ...
    logger.info(f"Filtering adata for {subset_name} genes in split {iteration_num}")
...
def prep_gene_classifier_all_data(data, targets, labels, max_ncells, num_proc):
    ...
    # filter dataset for examples containing classes for this split
    logger.info("Filtering training adata for genes to classify.")
...
def balance_attr_splits(
    ...
    for i in range(max_trials):
        if not all(
            ...
            logger.error(
                f"Cannot balance by {attr_to_split} while retaining at least 1 occurrence of each {state_key} class in both adata splits. "
            )
            raise
        eval_base = []
        ...
        for attr in attr_to_balance:
            ...
            if all(isinstance(item, (int, float)) for item in train_attr + eval_attr):
                ...
                logger.error(
                    f"Inconsistent adata types in attribute {attr}. "
