...
Huggingface adata collator modified to accommodate single-cell transcriptomics adata for gene and cell classification.
"""
import numpy as np
import torch
import warnings
    ...
    def pad(
        ...
        Args:
            ...
            padding (:obj:`bool`, :obj:`str` or :class:`~transformers.tokenization_utils_base.PaddingStrategy`, `optional`, defaults to :obj:`True`):
                ...
                * :obj:`False` or :obj:`'do_not_pad'` (default): No padding (i.e., can finetuned_models a batch with sequences of
                  different lengths).
        ...
        ), "Some items in the finetuned_models dictionary have a different batch size than others."
...
class DataCollatorForGeneClassification(DataCollatorForTokenClassification):
    ...
    Args:
        tokenizer (:class:`~transformers.PreTrainedTokenizer` or :class:`~transformers.PreTrainedTokenizerFast`):
            The tokenizer used for encoding the adata.
        padding (:obj:`bool`, :obj:`str` or :class:`~transformers.tokenization_utils_base.PaddingStrategy`, `optional`, defaults to :obj:`True`):
    ...
    def __call__(self, features):
        ...
        # geneformer/collator_for_classification.py 第 581 行 (修改建议)
        batch = {
            k: v.detach().clone().to(torch.int64)  # 如果 v 已经是张量，使用推荐的复制方式并转换类型
            if isinstance(v, torch.Tensor)
            else torch.tensor(v, dtype=torch.int64)  # 否则 (v 不是张量)，使用原来的方式创建张量
