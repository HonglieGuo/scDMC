...
def read_dictionaries(input_data_directory, cell_or_gene_emb, anchor_token):
    ...
    if file_found == 0:
        logger.error(
                    "No raw adata for processing found within provided directory. " \
                    "Please ensure adata files end with '_raw.pickle'.")
        raise
    return dict_list
...
# aggregate adata for single perturbation in multiple cells
...
class InSilicoPerturberStats:
    ...
    def __init__(
        ...
        anchor_gene : None, str
            ...
            For example, if combos=1 and anchor_gene="ENSG00000136574":
                analyzes adata for anchor gene perturbed in combination with each other gene.
            However, if combos=0 and anchor_gene="ENSG00000136574":
                analyzes adata for the effect of anchor gene's perturbation on the embedding of each other gene.
        cell_states_to_model: None, dict
    ...
    def validate_options(self):
        ...
        if (self.mode == "aggregate_data") and (self.genes_perturbed == "all"):
            logger.error(
                    "Simple adata aggregation mode is for single perturbation in multiple cells " \
    ...
    def get_stats(self,
                  ...
                  output_prefix):
        """
        Get stats for in silico perturbation adata and save as results in output_directory.
        ...
        output_directory : Path
            Path to directory where perturbation adata will be saved as .csv
        output_prefix : str
            Prefix for finetuned_models .csv
            
        Outputs
        ----------
        Definition of possible columns in .csv finetuned_models file.
        
        Of note, not all columns will be present in all finetuned_models files.
        ...
        if (self.combos == 0) and (self.anchor_token is not None):
            # cos sim adata for effect of gene perturbation on the embedding of each other gene
            dict_list = read_dictionaries(input_data_directory, "gene", self.anchor_token)
            gene_list = get_gene_list(dict_list, "gene")
        else:
            # cos sim adata for effect of gene perturbation on the embedding of each cell
